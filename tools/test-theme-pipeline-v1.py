...
assert spec['formatVersion']==3
assert set(spec['packs'])=={'board','directions','rigid','chrome'}
...
print(json.dumps({'themePipelineV2':'passed','slots':len(ids),'required':sum(1 for s in spec['slots'] if s.get('required',True)),'optional':sum(1 for s in spec['slots'] if not s.get('required',True))}))
