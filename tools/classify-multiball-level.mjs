...
function analyzeState(s){
 ...
 return{status:'ok',raw,optimalSolution:solved.path,metrics:{optimalMoves:L,baselineMoves:B,detourMoves:det,directionChanges:turns,setupMoves:setup,retreatMoves:retreat,conflictMoves:conflict,firstExitMove:firstExit||L,secondPhaseMoves:L-(firstExit||L),counterIntuitiveFirstExit:counter,searchedStates:solved.states}};
}
function classifyRecord(r){const s=stateFromRecord(r),a=analyzeState(s);if(a.status!=='ok')return a;const bounds=calibration[s.width+'x'+s.height];if(!bounds)throw Error('No calibration for '+s.width+'x'+s.height);return{...a,difficulty:1+bounds.filter(x=>a.raw>=x).length,model:'puzzle-v3-multiball'}}
