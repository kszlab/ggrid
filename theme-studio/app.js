...
const stages=['MOOD','TARGET','SHEETS','BUILD','QA','RELEASE'];
...
function renderPanel(){if(tab==='overview')return overview();if(tab==='mood')return stageImage('mood','Hangulatterv','mood');if(tab==='target')return stageImage('target','Render célkép','target');if(tab==='sheets')return sheets();if(tab==='build')return build()}
function overview(){panel(`<div class="card"><h3>Pilotprojekt</h3><dl class="meta"><dt>Állapot</dt><dd>${current.computedStage}</dd><dt>Projektverzió</dt><dd>${current.projectVersion}</dd><dt>Strict pipeline</dt><dd>${current.strict?'igen':'nem'}</dd><dt>Leírás</dt><dd>${esc(current.description||'')}</dd></dl></div><div class="card"><h3>Migráció</h3><p>${current.migration?.notes||'Új Theme Studio projekt.'}</p><div id="external"></div></div>`);api('/api/themes/'+current.id+'/external-sources').then(x=>{$('#external').innerHTML='<b>Örökölt források:</b><br>'+x.sources.map(s=>`<code>${s.path}</code> (${Math.round(s.size/1024)} KB)<br>`).join('')})}
