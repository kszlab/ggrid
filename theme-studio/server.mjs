...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(req.method==='POST'&&m){const id=m[1],p0=await loadProject(id);for(const v of Object.values(p0.assetOverrides||{})){v.status='approved';v.approvedAt=now()}await save(projectPath(id),{...p0,computedStage:undefined});const p1=await loadProject(id);p1.stages.sheets.approvalMode='asset-pack-v3';await syncPipelineApproval(id,p1,'sheets');return json(res,200,await buildTheme(id,{capture:false,quick:true}))}
