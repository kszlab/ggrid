...
const AudioManager=(()=>{
 ...
 function startAmbient(){stopAmbient();if(muted||!themeAudio?.ambient?.enabled)return;const p=themeAudio.profile,vol=Math.min(.04,themeAudio.ambient.volume||.02);const pulse=()=>{if(muted)return;const map={haunted:[110,165],city:[55,82],orbital:[48,96],abyss:[42,63]},q=map[p]||[70,105];tone(q[0],2.8,'sine',vol);tone(q[1],2.2,'sine',vol*.45,.35)};pulse();ambientTimer=setInterval(pulse,4200)}
 function themed(kind,fallback){if(!themeAudio)return fallback();const p=themeAudio.profile;const m={haunted:{move:[150,.13,'triangle'],blocked:[82,.16,'square'],freeze:[880,.32,'sine'],exit:[330,.5,'sine'],win:[660,.7,'sine']},city:{move:[105,.12,'sawtooth'],blocked:[240,.16,'square'],freeze:[520,.15,'square'],exit:[740,.3,'sine'],win:[880,.45,'triangle']},orbital:{move:[180,.1,'square'],blocked:[70,.2,'sawtooth'],freeze:[1200,.3,'sine'],exit:[540,.45,'sine'],win:[1080,.55,'sine']},abyss:{move:[75,.2,'sine'],blocked:[52,.28,'sine'],freeze:[760,.4,'sine'],exit:[310,.5,'sine'],win:[620,.7,'sine']}}[p]?.[kind];if(!m)return fallback();tone(m[0],m[1],m[2],.04,0,m[0]*.72)}
