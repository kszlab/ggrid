...
const AudioManager=(()=>{
 ...
 function startAmbient(){stopAmbient();if(muted||!themeAudio?.ambient?.enabled)return;const p=themeAudio.profile,vol=Math.min(.04,themeAudio.ambient.volume||.02);const pulse=()=>{if(muted)return;const map={haunted:[110,165],city:[55,82],orbital:[48,96],abyss:[42,63],clockwork:[46,92],neon:[40,80],zen:[98,147]},q=map[p]||[70,105];
if(p==='microchip'){const notes=[262,330,392,494,392,330,294,370];notes.forEach((n,i)=>tone(n,.16,'square',vol*.7,i*.19));tone(65,1.7,'sine',vol*.42);return}
if(p==='zen'){tone(98,3.5,'sine',vol*.75);tone(147,2.8,'sine',vol*.35,.5);return}
tone(q[0],2.8,'sine',vol);tone(q[1],2.2,'sine',vol*.45,.35)};pulse();ambientTimer=setInterval(pulse,4200)}
